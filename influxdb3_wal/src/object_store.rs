...
use crate::{SnapshotDetails, Wal, WalConfig, WalContents, WalFileNotifier, WalFileSequenceNumber, WalOp, WriteBatch};
use bytes::Bytes;
...
impl WalObjectStore {
    ...
    async fn write_op(&self, op: WalOp) -> crate::Result<(), crate::Error> {
        let (tx, rx) = oneshot::channel();
        self.flush_buffer
            .lock()
            .await
            .wal_buffer
            .buffer_op_with_response(op, tx);
...
impl FlushBuffer {
    ...
    async fn flush_buffer_into_contents_and_responses(
        ...
        println!(
            "adding wal period: {} {} {}",
            wal_contents.wal_file_number.0,
...
impl WalBuffer {
    ...
    fn buffer_op_with_response(&mut self, op: WalOp, response: oneshot::Sender<WriteResult>) {
        self.write_op_responses.push(response);
        self.buffer_op_unconfirmed(op).unwrap();
    }
    ...
    fn into_wal_contents_and_responses(self) -> (WalContents, Vec<oneshot::Sender<WriteResult>>) {
        ...
        let write_batches = self
            .database_to_write_batch
            .into_iter()
            .map(|(_, batch)| WalOp::Write(batch))
            .collect();
...
mod tests {
    use super::*;
    use crate::{TableChunk, FieldData, Row, TableChunks, Field};
