...
impl Persister for PersisterImpl {
        ...
        match catalog_path {
            ...
            Some(path) => {
                ...
                Ok(Some(PersistedCatalog {
                    wal_file_sequence_number: WalFileSequenceNumber::new(parsed_number),
                    catalog,
                }))
            }
        }
    ...
    async fn persist_catalog(&self, wal_file_sequence_number: WalFileSequenceNumber, catalog: Catalog) -> Result<()> {
    ...
    async fn persist_snapshot(&self, persisted_snapshot: &PersistedSnapshot) -> Result<()> {
        let segment_file_path = SnapshotInfoFilePath::new(persisted_snapshot.wal_file_sequence_number);
...
mod tests {
    ...
    async fn persist_and_load_newest_catalog() {
        ...
        assert_eq!(catalog.wal_file_sequence_number, WalFileSequenceNumber::new(1));
    ...
    async fn persist_and_load_parquet_bytes() {
        ...
        let path = ParquetFilePath::new("db_one", "table_one", Utc::now(), WalFileSequenceNumber::new(1), 1);
